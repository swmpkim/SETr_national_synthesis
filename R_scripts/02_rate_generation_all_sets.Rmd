...
for(i in seq_along(reserves)){
  ...
  for(j in seq_along(comp_type)){
    ...
    p <- ggplot(to_plot_sub) +
      ...
      #     subtitle = paste0("(n = ", nsets, ")")) +   # labels don't work when the icons are so small
      theme_void() +   
    ...
    # save the plot
    file_out <- paste0(res_to_plot, "_", comp_to_plot, ".png")
    out_path <- here::here("R_output", "figures", "pie_charts", file_out)
    ggsave(out_path, bg = "transparent")
  }
}
