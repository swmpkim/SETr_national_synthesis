...
```{r}
knitr::kable(lmm_out_all, digits = 4)
```

The biggest difference we see in rates between model type is:  
```{r}
max(lmm_out_all$rate_diff)
```

The biggest difference in CI width between modely types is:  
```{r}
